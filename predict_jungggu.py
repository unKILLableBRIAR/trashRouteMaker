...
junggu = df[df['city_gn_gu_nm'] == '중구'].copy()

# 날짜 열 생성
junggu['ds'] = pd.to_datetime({
    'year': junggu['year'],
    'month': junggu['mt'],
    'day': junggu['dt']
})
...
junggu = junggu.dropna(subset=['dscamt'])
junggu['y'] = junggu['dscamt'] / 1000
junggu['month'] = junggu['ds'].dt.month
...
# 날씨 데이터 병합
junggu = pd.merge(junggu, rain_data, on='month', how='left')
junggu = pd.merge(junggu, hot_data, on='month', how='left')
junggu = pd.merge(junggu, snow_data, on='month', how='left')
...
model.fit(junggu[['ds', 'y', 'rain_days', 'hot_days', 'snow_days']])
...
# 9. 이미지 저장
fig1.savefig("output/predict_plot_weather_junggu.png", dpi=300, bbox_inches='tight')
fig2.savefig("output/components_plot_weather_junggu.png", dpi=300, bbox_inches='tight')
