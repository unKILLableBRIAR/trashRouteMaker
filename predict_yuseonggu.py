...
yuseonggu = df[df['city_gn_gu_nm'] == '유성구'].copy()

# 날짜 열 생성
yuseonggu['ds'] = pd.to_datetime({
    'year': yuseonggu['year'],
    'month': yuseonggu['mt'],
    'day': yuseonggu['dt']
})
...
yuseonggu = yuseonggu.dropna(subset=['dscamt'])
yuseonggu['y'] = yuseonggu['dscamt'] / 1000
yuseonggu['month'] = yuseonggu['ds'].dt.month
...
# 날씨 데이터 병합
yuseonggu = pd.merge(yuseonggu, rain_data, on='month', how='left')
yuseonggu = pd.merge(yuseonggu, hot_data, on='month', how='left')
yuseonggu = pd.merge(yuseonggu, snow_data, on='month', how='left')
...
model.fit(yuseonggu[['ds', 'y', 'rain_days', 'hot_days', 'snow_days']])
...
plt.title("유성구 쓰레기 배출량 예측 (날씨 반영)")
plt.xlabel("날짜")
plt.ylabel("배출량 (kg)")
plt.tight_layout()
plt.grid(True)
