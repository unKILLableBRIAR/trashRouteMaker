...
daedeokgu = daedeokgu.dropna(subset=['dscamt'])
daedeokgu['y'] = daedeokgu['dscamt'] / 1000
daedeokgu['month'] = daedeokgu['ds'].dt.month
...
# 날씨 데이터 병합
daedeokgu = pd.merge(daedeokgu, rain_data, on='month', how='left')
daedeokgu = pd.merge(daedeokgu, hot_data, on='month', how='left')
daedeokgu = pd.merge(daedeokgu, snow_data, on='month', how='left')
...
model.fit(daedeokgu[['ds', 'y', 'rain_days', 'hot_days', 'snow_days']])
...
plt.title("대덕구 쓰레기 배출량 예측 (날씨 반영)")
plt.xlabel("날짜")
plt.ylabel("배출량 (kg)")
plt.tight_layout()
plt.grid(True)
...
# 9. 이미지 저장
fig1.savefig("output/predict_plot_weather_daedeokgu.png", dpi=300, bbox_inches='tight')
fig2.savefig("output/components_plot_weather_daedeokgu.png", dpi=300, bbox_inches='tight')
