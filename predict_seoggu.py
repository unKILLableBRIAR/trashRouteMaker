...
seoggu = df[df['city_gn_gu_nm'] == '서구'].copy()

# 날짜 열 생성
seoggu['ds'] = pd.to_datetime({
    'year': seoggu['year'],
    'month': seoggu['mt'],
    'day': seoggu['dt']
})
...
seoggu = seoggu.dropna(subset=['dscamt'])
seoggu['y'] = seoggu['dscamt'] / 1000
seoggu['month'] = seoggu['ds'].dt.month
...
# 날씨 데이터 병합
seoggu = pd.merge(seoggu, rain_data, on='month', how='left')
seoggu = pd.merge(seoggu, hot_data, on='month', how='left')
seoggu = pd.merge(seoggu, snow_data, on='month', how='left')
...
model.fit(seoggu[['ds', 'y', 'rain_days', 'hot_days', 'snow_days']])
